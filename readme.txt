...
12.Commits on Dec 7, 2016
AbstractFactory_Main
本次重构主要对主界面部分进行修改，应用抽象工厂模式进行重构。
考虑到不同的用户可能需要不同的功能组合，同时为了让主界面更加多样化，将抽象类StartInterface作为各主界面类的父类。如果想设计自己的主界面可以新建一个继承自该类的子类，如MainStartInterace，设计和实现界面以及功能点排列就可以得到一个完整可用的独立的主界面类。